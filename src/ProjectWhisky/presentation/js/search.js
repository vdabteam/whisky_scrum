jQuery(function(){
     ...
     */
    jQuery("#score_range").editRangeSlider({step: 0.5, defaultValues:{min: 0, max: 10}, bounds:{min: 0, max: 10}, wheelMode: "scroll", wheelSpeed: 10, 
                                        formatter:function(val){
                                            var decimal = val - Math.round(val);
                                            return decimal == 0 ? val.toString() + ".0" : val.toString();
                                        }
    });
    jQuery("#strength_range").editRangeSlider({step: 1, defaultValues:{min: 1, max: 100}, bounds:{min: 1, max: 100}, wheelMode: "scroll", wheelSpeed: 10});
    jQuery("#age_range").editRangeSlider({step: 1, defaultValues:{min: 1, max: 70}, bounds:{min: 1, max: 70}, wheelMode: "scroll", wheelSpeed: 10});
